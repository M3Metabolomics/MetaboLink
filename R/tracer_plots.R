...
format4ggplot <- function(data, sequence, isotopologues) {
  ...
  # Order isotopologues by the numeric part after '+' (A+0, A+1, A+2, A+10)
...
# Plot metabolite & sample "Isotopologue distribution — [metabolite] (Normalized to A.0)"
...
plotStackedIsotopologues <- function(data, plot_settings) {
  ...
  # try to extract numeric part after '+' to sort numerically
  nums <- suppressWarnings(as.numeric(gsub(".*\\+(\\d+).*", "\\1", iso_levels)))
  if (all(!is.na(nums))) {
